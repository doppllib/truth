...
#include "ComGoogleCommonBasePredicate.h"
#include "ComGoogleCommonBasePredicates.h"
...
- (NSString *)getDisplaySubject {
  ...
  if (customName_ != nil) {
    return JreStrcat("$$", customName_, ([((NSString *) nil_chk(formatted)) isEmpty] ? @"" : JreStrcat("$$$", @" (<", formatted, @">)")));
  }
  else {
...
- (NSString *)actualCustomStringRepresentation {
  return NSString_valueOf_(ComGoogleCommonTruthSubject_actual(self));
}
